...
let barSpeed = 10; //velocidad de llenar barra 100 para test rapido 
  ...
  if (i == 0) {
    ...
    function frame() {
      if (width >= 1000) {
        ...
        i = 0;
        width = 1;
        window.document.getElementById('levelUp').style.display="block";
...
function balancea(){
  ...
  for(var i in mainArr){
    mainArr[i][2].gain.linearRampToValueAtTime(1/mainArr.length, ctx.currentTime+2);
    //ratio=ratio-0.001;
  }
